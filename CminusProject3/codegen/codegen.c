...
int emitIf(DList instList, SymTable symtab, int regIndex){
        char label[20];
        snprintf(label,"19",".L%d",count++);
...
void emitEndIf(DList instList, SymTable symtab, int labelNum){
        char *inst = nssave(2,SymGetFieldByIndex(symtab,labelNum,SYM_NAME_FIELD),":\tnop");
        dlinkAppend(instList, dlinkNodeAlloc(inst));
}
...
int emitThen(DList instList,SymTable symtab, int labelNum){
        char label[20];
        snprintf(label,19,".L%d",count++);

        char *inst = nssave(2,"\tjmp\t",label);
        dlinkAppend(instList, dlinkNodeAlloc(inst));
        emitEndIf(instList,symtab, labelNum);
...
int emitWhileLoop(DList instList, SymTable symtab, int regNum){

        char label[20];
        snprintf(label, "19",".L%d",count++);

        char *regName = getIntegerRegisterName(regNum);
